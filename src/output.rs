use crate::model::{NavigationRecord, RelatedRecord, RelatedTestRecord, Symbol};
...
pub fn related_json(records: &[RelatedRecord]) -> anyhow::Result<String> {
    Ok(serde_json::to_string_pretty(records)?)
}

pub fn related_plain(records: &[RelatedRecord]) -> String {
    records
        .iter()
        .map(|record| {
            format!(
